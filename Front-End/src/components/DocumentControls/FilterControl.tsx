...
export const filterDocuments = (
  ...
  const filteredDocuments = documents.filter((d) => {
    ...
    const isCoauthor = cloudDocument?.coauthors.some(
      (coauthor) => coauthor.id === user?.id
    );
    const isCollaborator = cloudDocument?.revisions.some(
      (revision) => revision.author.id === user?.id
    );
    const isOthers =
      !isLocalOnly && !isAuthor && !isCoauthor && !isCollaborator;
    ...
    const showCollaborator =
      !!(value & (1 << 9)) && !!isCollaborator && !isAuthor && !isCoauthor;
          ...
          sx={{
            color:
              value & (1 << option.key)
                ...
                : "text.secondary",
            backgroundColor:
              value & (1 << option.key) ? "primary.main" : "action.selected",
            "& .MuiTab-label": {
              display: {
