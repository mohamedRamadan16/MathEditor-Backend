...
const NewDocument: React.FC<{ cloudDocument?: CloudDocument }> = ({
  ...
  const [input, setInput] = useState<Partial<DocumentCreateInput>>({
    ...
    private: false, // Default to not private
    collab: false, // Default to not collab
  });
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload: DocumentCreateInput = {
        ...
        head: uuidv4(),
        name,
        handle: input.handle || "", // Let the Redux action handle unique handle generation
        data,
        createdAt,
        updatedAt: createdAt,
      };
      ...
      if (response.type === actions.createLocalDocument.fulfilled.type) {
        let finalHandle = payload.handle || payload.id;

        // Only create cloud document if explicitly requested and not already done
        if (saveToCloud) {
          const cloudResponse = await dispatch(
          ...
          if (
            ...
            // Use the cloud document's handle for navigation
            const cloudDocument = cloudResponse.payload as any;
            finalHandle = cloudDocument.handle;
          }
        }

        const href = `/edit/${finalHandle}`;
        navigate(href);
      }
    } catch (error) {
        ...
        >
          ...
          {saveToCloud && (
            <>
              ...
              />
              <FormHelperText>
                Published documents are visible to everyone and appear in the
                  ...
                  <Checkbox
                    ...
                    onChange={() =>
                      updateInput({
                        ...
                        published: input.private ? input.published : false,
          ...
          <Button
            type="submit"
            disabled={
              !!(baseId && !base) || validating || hasErrors || isSubmitting
            }
            fullWidth
            variant="contained"
            startIcon={<Add />}
