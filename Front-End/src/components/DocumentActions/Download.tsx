...
import { BackupDocument, UserDocument, EditorDocumentRevision } from "@/types";
...
const DownloadDocument: React.FC<{
  ...
  const getEditorDocument = async () => {
    if (isLocal) {
      ...
      if (response.type === actions.getCloudDocument.fulfilled.type) {
        const { cloudDocument, ...editorDocument } =
          response.payload as ReturnType<
  ...
  const getBackupDocument = async () => {
    ...
    if (
      ...
      const revisions = revisionsResponse.payload as EditorDocumentRevision[];
      backupDocument.revisions = revisions.filter(
        (revision) => revision.id !== editorDocument.head
      );
    }
