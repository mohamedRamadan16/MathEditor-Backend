...
import {
  ...
  $isGraphNode,
  GraphNode,
} from "@/editor/nodes/GraphNode";
import {
  $createSketchNode,
  $isSketchNode,
  SketchNode,
} from "@/editor/nodes/SketchNode";
import {
  $createStickyNode,
  $isStickyNode,
  StickyNode,
...
export const SKETCH: TextMatchTransformer = {
  ...
  replace: (textNode, match) => {
    const [, src] = match;
    const id = `sketch-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    const sketchNode = $createSketchNode({
      src,
      width: 0,
      height: 0,
      id,
  ...
  replace: (
    ...
    if (!children && linesInBetween) {
      ...
      const id = `math-${Date.now()}-${Math.random()
        .toString(36)
        .substr(2, 9)}`;
      const mathNode = $createMathNode("", "", id);
      let math: string;
